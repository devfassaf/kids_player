...
| Bluetooth / headset / watch / car control (v1.0.84, skip fixed v1.0.85) | **DEVICE ONLY.** Play any video or song (screen ON, background-playback OFF). A Bluetooth headset/watch/car should: **play/pause** (its ⏯) and **change track** with its ⏮/⏭ (next/previous ITEM in the grid order, gifts skipped, no wrap — NOT ±10s). ⚠️ **v1.0.85: a smartwatch/car only SENDS the transport actions the session ADVERTISES** — skip was silent on v1.0.84 until `ACTION_SKIP_TO_NEXT/PREVIOUS` were advertised. Verify the WATCH's next/previous actually changes the song, not just pause. The lock-screen widget/car display should show the title + transport (skip triangles change track; the ±10 ring buttons seek). Repeat with a **YouTube** video (screen on) — same controls work. ⚠️ With the screen OFF, a YouTube video cannot keep playing (Android throttles the WebView) — that is expected. The media **notification now shows during ALL playback** (not just background mode); confirm it has no way to open/leave the app (kiosk-safe). Files with background-playback ON still keep playing + controllable when backgrounded. |
| ⚠️ Headset/hands-free forward+back keys (v1.0.92) | **DEVICE ONLY — and the reported bug: v1.0.84/85 claimed this and it was never actually pressed.** Play a song/video and connect a Bluetooth headset or car hands-free. (a) Its **⏮/⏭** must change VIDEO (next/previous item in the grid order, gifts skipped, no wrap) — not seek. (b) Its **⏪/⏩**: a **short** press changes video, **holding** it ~0.5s moves ±10s inside the video instead. (c) A **one-button** hands-free: 1 press pauses/resumes INSTANTLY, 2 rapid presses change video (no triple-press = previous, by design). (d) The notification's own drawn ⏪10/⏩10 must STILL be ±10s. (e) Press ⏭ while the video FLOATS in the mini-player (BACK with the PiP setting on) → the track changes and it stays floating, never jumping back to the watch screen. ⚠️ Some kits swallow a double-press as "redial" and never send it to Android — that is the device, not the app. |
| Media plays on an approved site (v1.0.83) | **DEVICE ONLY** — the viewer is native. Approve a curated video site whose videos stream from a CDN (anafeam-kids.co.il — HLS from `*.b-cdn.net`) **without** enabling external content. In **child mode**, open a video → **it must now play** (v1.0.83 auto-allows media on any approved page). Confirm a non-media third-party thing is still blocked (the page's own scripts/ads don't leak) and that navigation is still locked to approved pages. |
| ⚠️ Disconnect must NOT wipe the library (v1.0.79) | **DEVICE ONLY, and this was the worst data-loss bug the app shipped.** Have at least one profile with real videos and at least one approved site. Parent → approved sites → tap a site's **"התנתקות"** (log out) → confirm. Then check **every** profile's home: all videos and lists must still be there. The bug was `clearSiteData` calling `WebStorage.getInstance().deleteAllData()` (a per-app singleton), which erased every profile's IndexedDB library. Also press the **🗑️/"הסר"** button — it only removes the site row and must likewise leave all videos intact. The invariants guard bans the wipe call, but only a device proves the actual disconnect clears nothing but the site's login cookies. |
| CDN video on an approved site (v1.0.78) | ⚠️ **SUPERSEDED by v1.0.83** — media now auto-plays on any approved site, so the old "approve WITHOUT external content → video is blank" no longer holds (that was the reported bug; see the v1.0.83 row above). The `allowExternal` per-rule toggle and the child's blocked-page external-content step still exist for NON-media third-party content, and remain worth a device check: enabling external content must let a genuinely embedded non-media resource load, and the blocked-page "הורים" → code flow's second question "לאפשר סרטונים ותוכן מאתרים אחרים?" must still appear. |
