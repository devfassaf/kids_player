...
| CDN video on an approved site (v1.0.78) | **DEVICE ONLY** — the viewer is native. Approve a curated video site whose videos come from a CDN (anafeam-kids.co.il — its videos are HLS from `*.b-cdn.net`). Approve it **WITHOUT external content** → open a `/watch/` page → the video must be BLANK/not play (the reported bug). Now enable external content — either re-approve, or Parent → approved sites → the site's "תוכן חיצוני" toggle — → reopen the page → **the video must play**. Also check the **child's blocked-page flow**: if the child hits a blocked link, "הורים" → code → pick the slice → the SECOND question "לאפשר סרטונים ותוכן מאתרים אחרים?" must appear, and choosing it must make videos work. |
