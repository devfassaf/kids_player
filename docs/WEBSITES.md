...
⚠️ **"התנתקות" מאתר מנקה רק את ההתחברות של אותו host — הספרייה של כל פרופיל שורדת שלמה**
(רגרסיית v1.0.79: המשמר חוסם את הקוד, אבל את ההתנהגות בפועל אפשר להוכיח רק על מכשיר —
התחברו לאתר, לחצו התנתקות, ובדקו שכל פרופיל עדיין מלא בסרטונים).

## 8. מגבלות ידועות

- **SSO במצב ילד נחסם** — לפי התכנון. ההתחברות היא פעולה הורית.
- ⚠️ **`clearSiteData` מנקה עוגיות בלבד — לעולם לא Web Storage** (v1.0.79). זו הייתה תקלת
  איבוד-הנתונים החמורה ביותר שהאפליקציה שחררה: הכפתור "התנתקות" (לשעבר "ניתוק") קרא
  ל-`WebStorage.getInstance().deleteAllData()`, שהוא **סינגלטון ברמת האפליקציה** — הוא מוחק
  את ה-Web Storage של **כל** ה-origins, כולל ה-IndexedDB של ה-WebView של גשר Capacitor, שם
  `db.js` שומר את כל הספרייה של כל הפרופילים. כך "התנתקות מאתר אחד" מחקה בשקט את כל הסרטונים
  של כל הפרופילים (הפרופילים/PIN/הגדרות שרדו רק כי הם ב-Preferences הנייטיביים, לא ב-Web
  Storage). **אין API של Web Storage לפי host באנדרואיד** (`deleteOrigin` deprecated ולא
  מכסה IndexedDB באמינות), אז מנקים רק את **העוגיות** של ה-host — עיקר האימות להתנתקות —
  ומקבלים שסשן מבוסס-localStorage בלבד עלול להישאר. משמר `invariants.test.mjs` אוסר
  `WebStorage`/`deleteAllData` בכל מקום בפלאגין. סשן שנשאר קטן לאין ערוך ממחיקת ספריית המשפחה.
